...
/-! #### Groupoid API for `C0SectorMorphism`

`C0SectorMorphism Ω Ω'` bundles a `c0Rel` witness with chosen coherent
`SectorEquivData`.  The canonical `ITPSector`-level transport `toEquiv`
depends only on the `c0Rel` witness via `c0Transport`, so it satisfies
clean refl / symm / trans identities even though the auxiliary `data`
field is non-canonical at the `regionMap` level. -/

namespace C0SectorMorphism

/-- The identity morphism on `Ω`, using the non-canonical
`SectorEquivData.refl` for the coherent transport data. -/
noncomputable def refl (Ω : UnitFamily H) : C0SectorMorphism Ω Ω where
  rel := c0Rel.refl Ω
  data := SectorEquivData.refl Ω

/-- Symmetric morphism. -/
noncomputable def symm {Ω Ω' : UnitFamily H}
    (m : C0SectorMorphism Ω Ω') : C0SectorMorphism Ω' Ω where
  rel := m.rel.symm
  data := m.data.symm

/-- Composition of two `C0SectorMorphism`s. -/
noncomputable def trans {Ω Ω' Ω'' : UnitFamily H}
    (m₁ : C0SectorMorphism Ω Ω') (m₂ : C0SectorMorphism Ω' Ω'') :
    C0SectorMorphism Ω Ω'' where
  rel := m₁.rel.trans m₂.rel
  data := m₁.data.trans m₂.data

/-- Construct a `C0SectorMorphism` from a bare `c0Rel` witness using the
non-canonical `SectorEquivData.ofC0Rel`. -/
noncomputable def ofC0Rel {Ω Ω' : UnitFamily H}
    (h : c0Rel Ω Ω') : C0SectorMorphism Ω Ω' where
  rel := h
  data := SectorEquivData.ofC0Rel h

/-- The canonical `ITPSector`-level transport associated to a
`C0SectorMorphism`.  Defined via `c0Transport` on the bundled `c0Rel`
witness; in particular it does **not** depend on the auxiliary `data`
field and so satisfies clean groupoid identities. -/
noncomputable def toEquiv {Ω Ω' : UnitFamily H}
    (m : C0SectorMorphism Ω Ω') :
    ITPSector (H := H) Ω ≃ₗᵢ[ℂ] ITPSector (H := H) Ω' :=
  c0Transport m.rel

@[simp]
theorem toEquiv_refl (Ω : UnitFamily H) :
    (refl (H := H) Ω).toEquiv = LinearIsometryEquiv.refl ℂ _ :=
  c0Transport_refl Ω

@[simp]
theorem toEquiv_symm {Ω Ω' : UnitFamily H}
    (m : C0SectorMorphism Ω Ω') :
    m.symm.toEquiv = m.toEquiv.symm :=
  (c0Transport_symm m.rel).symm

@[simp]
theorem toEquiv_trans {Ω Ω' Ω'' : UnitFamily H}
    (m₁ : C0SectorMorphism Ω Ω') (m₂ : C0SectorMorphism Ω' Ω'') :
    (m₁.trans m₂).toEquiv = m₁.toEquiv.trans m₂.toEquiv :=
  (c0Transport_trans m₁.rel m₂.rel).symm

@[simp]
theorem ofC0Rel_toEquiv {Ω Ω' : UnitFamily H} (h : c0Rel Ω Ω') :
    (ofC0Rel h).toEquiv = c0Transport h := rfl

end C0SectorMorphism

/-! #### Bundled group representations on `ITPSector Ω`

The mathlib instance `LinearIsometryEquiv.instGroup` gives the type
`ITPSector Ω ≃ₗᵢ[ℂ] ITPSector Ω` a group structure under composition.
Bundled `G`-representations on `ITPSector Ω` therefore have the
standard monoid-homomorphism shape, recorded here under the abbreviation
`ITPGroupRepresentation`. -/

/-- Bundled `G`-representation on `ITPSector Ω` by isometric automorphisms.

A `MonoidHom` from `G` into the isometric automorphism group of
`ITPSector Ω` packages the per-element unitary action together with the
group representation laws

```
U_1 = id,    U_{g · h} = U_g ∘ U_h.
```

The trivial representation is `1 : ITPGroupRepresentation G Ω`. -/
abbrev ITPGroupRepresentation (G : Type*) [Monoid G] (Ω : UnitFamily H) :
    Type _ :=
  G →* (ITPSector (H := H) Ω ≃ₗᵢ[ℂ] ITPSector (H := H) Ω)

namespace ITPGroupRepresentation

variable {G : Type*} [Monoid G] {Ω : UnitFamily H}

/-- The trivial representation: every group element acts as the identity. -/
noncomputable def trivial : ITPGroupRepresentation G Ω := 1

@[simp]
theorem trivial_apply (g : G) :
    (trivial : ITPGroupRepresentation G Ω) g = 1 := rfl

end ITPGroupRepresentation
