...
noncomputable def extendDense [CompleteSpace Y] (hs : Dense S) (f : S → Y) : X → Y :=
  hs.extend f
...
lemma extended_isometry_is_continuous [CompleteSpace Y] [T2Space Y] (hs : Dense S) (f : S → Y) (hf : Isometry f) :
    Continuous (extendDense hs f) := by
  unfold extendDense
...
lemma extended_isometry_is_isometry [CompleteSpace Y] [T2Space Y] (hs : Dense S) (f : S → Y) (hf : Isometry f) :
    Isometry (extendDense hs f) := by
  intro x y
  -- The set of pairs where distance is preserved is closed
  have h_closed : IsClosed {p : X × X | edist (extendDense hs f p.1)
                                              (extendDense hs f p.2) = edist p.1 p.2} := by
    have h_cont1 : Continuous (fun p : X × X => edist (extendDense hs f p.1) (extendDense hs f p.2)) :=
  ...
  have h_on_dense : ∀ x₁ x₂ : X, x₁ ∈ S → x₂ ∈ S →
      edist (extendDense hs f x₁) (extendDense hs f x₂) = edist x₁ x₂ := by
    intros x₁ x₂ hx₁ hx₂
  ...
  have : (x, y) ∈ {p : X × X | edist (extendDense hs f p.1) (extendDense hs f p.2) = edist p.1 p.2} := by
