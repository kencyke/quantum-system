...
For a von Neumann algebra `M` on `H` and a Hilbert space `H₁`, the **amplification**
`VonNeumannAlgebra.amplify H₁ M` is the von Neumann algebra on `H₁ ⊗̂ H` generated by
`1 ⊗ M = {amplifyRight x | x ∈ M}`; for `M = 𝓑(H)` it is `vnTensorRight`. For complete, nontrivial
`H₁` it is `1 ⊗ M` itself (`VonNeumannAlgebra.mem_amplify_iff`), via the slice lemma
`HilbertTensor.exists_amplifyRight_of_commutes`. Its commutant `B(H₁) ⊗̄ M′` is not formalised.

## Main definitions

* `HilbertTensor.vnTensorLeft`, `HilbertTensor.vnTensorRight`.
* `HilbertTensor.tmulRightL f` — the inclusion `H₂ → H₁ ⊗̂ H₂`, `z ↦ f ⊗ z`.
* `VonNeumannAlgebra.amplify H₁ M` — the von Neumann algebra generated by `1 ⊗ M`.
* `VonNeumannAlgebra.amplifyEquiv H₁ M` — the `⋆`-isomorphism `x ↦ 1 ⊗ x` of `M` onto
  `amplify H₁ M` (for complete, nontrivial `H₁`).
...
* `VonNeumannAlgebra.amplify_boundedLinearOperators` — `amplify H₁ 𝓑(H) = 1 ⊗̄ B(H)`
  (`HilbertTensor.vnTensorRight`).
* `VonNeumannAlgebra.amplifyRight_mem_amplify`,
  `VonNeumannAlgebra.amplifyRight_mem_commutant_amplify` — `1 ⊗ x ∈ amplify H₁ M` for `x ∈ M`, and
  `1 ⊗ x′ ∈ (amplify H₁ M)′` for `x′ ∈ M′`.
* `VonNeumannAlgebra.mem_amplify_iff`, `VonNeumannAlgebra.coe_amplify` — `amplify H₁ M = 1 ⊗ M`
  (for complete, nontrivial `H₁`).
* `VonNeumannAlgebra.inner_apply_eq_of_mem_generated` — two vector functionals that agree on a
  unital `⋆`-subalgebra `A` agree on the von Neumann algebra it generates (by the density form of
...
/-! ### The amplification `1 ⊗ M` of a von Neumann algebra -/

open scoped InnerProductSpace VonNeumannAlgebra HilbertTensor
open HilbertTensor (amplifyLeft amplifyRight amplifyRightₐ)
...
/-- **Vector functionals on a generated von Neumann algebra.** If the vector functionals of `Ξ`
and `Ξ′` agree on a unital `⋆`-subalgebra `A`, they agree on the von Neumann algebra `A″`
generated by `A`: `A″` is the weak-operator closure of `A`, and `y ↦ ⟪Ξ, y Ξ⟫` is
weak-operator continuous. -/
theorem inner_apply_eq_of_mem_generated (A : StarSubalgebra ℂ (K →L[ℂ] K)) {Ξ Ξ' : K}
    (h : ∀ x ∈ A, ⟪Ξ, x Ξ⟫_ℂ = ⟪Ξ', x Ξ'⟫_ℂ) {y : K →L[ℂ] K}
    (hy : y ∈ generated (A : Set (K →L[ℂ] K))) : ⟪Ξ, y Ξ⟫_ℂ = ⟪Ξ', y Ξ'⟫_ℂ := by
  have hstar : star (A : Set (K →L[ℂ] K)) = A :=
    Set.ext fun x => ⟨fun hx => by simpa using star_mem (s := A) hx,
      fun hx => Set.mem_star.mpr (star_mem (s := A) hx)⟩
  have hy' : y ∈ Set.centralizer (Set.centralizer (A : Set (K →L[ℂ] K))) := by
    rw [← coe_generated_of_star_eq hstar]
    exact hy
  have hmem : ContinuousLinearMapWOT.ofCLM y ∈
      closure (WeakOperatorTopology.Set.toWOT (H := K) (A.toNonUnitalStarSubalgebra : Set (K →L[ℂ] K))) := by
    rw [SOTClosedSubalgebra.wotClosure_eq_doubleCommutant A.toNonUnitalStarSubalgebra
      (InnerProductSpace.actsNondegenerately_of_one_mem A.one_mem)]
    exact hy'
  have hclosed : IsClosed {T : K →WOT[ℂ] K | ⟪Ξ, T Ξ⟫_ℂ = ⟪Ξ', T Ξ'⟫_ℂ} :=
    isClosed_eq (ContinuousLinearMapWOT.continuous_dual_apply Ξ (innerSL ℂ Ξ))
      (ContinuousLinearMapWOT.continuous_dual_apply Ξ' (innerSL ℂ Ξ'))
  exact closure_minimal (t := {T : K →WOT[ℂ] K | ⟪Ξ, T Ξ⟫_ℂ = ⟪Ξ', T Ξ'⟫_ℂ}) (fun T hT => h _ hT)
    hclosed hmem
...
variable (H₁) in
/-- The **amplification** of a von Neumann algebra `M` on `H` to `H₁ ⊗̂ H`: the von Neumann algebra
generated by `1 ⊗ M`. -/
...
theorem amplify_boundedLinearOperators :
    (𝓑(H) : VonNeumannAlgebra H).amplify H₁ = HilbertTensor.vnTensorRight := by
...
theorem amplifyRight_mem_commutant_amplify (hx : x ∈ M′) :
  ...
  refine ⟨?_, ?_⟩
  · rw [← HilbertTensor.amplifyRight_mul, ← HilbertTensor.amplifyRight_mul,
      mem_commutant_iff.mp hx y hy]
  · rw [HilbertTensor.amplifyRight_star, ← HilbertTensor.amplifyRight_mul,
      ← HilbertTensor.amplifyRight_mul, mem_commutant_iff.mp hx _ hy']
...
theorem amplifyLeft_mem_commutant_amplify (A : H₁ →L[ℂ] H₁) :
  ...
  refine ⟨?_, ?_⟩
  · rw [ContinuousLinearMap.mul_def, ContinuousLinearMap.mul_def]
    exact (HilbertTensor.amplifyLeft_comp_amplifyRight A x).symm
  · rw [HilbertTensor.amplifyRight_star, ContinuousLinearMap.mul_def, ContinuousLinearMap.mul_def]
...
theorem mem_amplify_iff [CompleteSpace H₁] [Nontrivial H₁] {y : H₁ ⊗̂ H →L[ℂ] H₁ ⊗̂ H} :
  ...
  have he : ‖(‖u‖⁻¹ : ℂ) • u‖ = 1 := by
    rw [norm_smul, norm_inv, Complex.norm_real, norm_norm,
      inv_mul_cancel₀ (norm_ne_zero_iff.mpr hu)]
  obtain ⟨x, rfl⟩ := HilbertTensor.exists_amplifyRight_of_commutes _ he y fun A => by
    rw [← ContinuousLinearMap.mul_def, ← ContinuousLinearMap.mul_def]
    exact mem_commutant_iff.mp (amplifyLeft_mem_commutant_amplify A) y hy
  refine ⟨x, ?_, rfl⟩
  rw [← commutant_commutant M, mem_commutant_iff]
  intro z hz
  apply HilbertTensor.amplifyRight_injective (H₁ := H₁)
  rw [HilbertTensor.amplifyRight_mul, HilbertTensor.amplifyRight_mul]
  exact (mem_commutant_iff.mp (amplifyRight_mem_commutant_amplify (H₁ := H₁) hz) _ hy).symm
...
noncomputable def amplifyEquiv [CompleteSpace H₁] [Nontrivial H₁] : M ≃⋆ₐ[ℂ] M.amplify H₁ :=
  StarAlgEquiv.ofBijective
    ...
    ⟨fun _ _ h => Subtype.ext (HilbertTensor.amplifyRight_injective (congrArg Subtype.val h)),
      fun y => by
        obtain ⟨x, hx, h⟩ := mem_amplify_iff.mp y.2
...
theorem amplifyEquiv_one [CompleteSpace H₁] [Nontrivial H₁] : amplifyEquiv H₁ M 1 = 1 :=
  map_one _
